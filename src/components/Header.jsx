...
export default function Header({ route, onDownload }) {
  ...
  return (
    <header className={`header ${scrolled ? "header--scrolled" : ""}`}>
      ...
      <nav id="site-nav" className={`nav ${menuOpen ? "nav--open" : ""}`} aria-label={t("nav_aria")}>
        {LINKS.map((l) => (
          <a
            key={l.key}
          ...
          >
            <Icon name={l.icon} size={15} className={`nav__link-icon nav__link-icon--${l.icon}`} />
            {t(l.key)}
      ...
      <div className="header__actions">
        ...
        <button
          ...
          title={lang === "fr" ? t("lang_switch_to_en") : t("lang_switch_to_fr")}
        >
          <span className="lang-btn__text">{lang === "fr" ? "EN" : "FR"}</span>
        </button>
        <a
